...
module.exports = {
    parse: function(qs, referer){
        //validate the query string
        if(!qs)
            return {error:"No query string was provided."};
    
        //separate libs and flags
        var flags = [];
        var libs = [];
        var refererBaseUrl = referer.replace(helpers.rx.baseUrlRegex, '')+'/';
        ...
        if(hasFlag("prod")){
            ...
            debug = referer && helpers.rx.isDebug.test(referer);
        }
